...
u_int8 util_array_check(u_int8 *array, u_int32 bitTotal)
{
        ...
        for (;i<bytenum;i++)
        {
                if (array[i])
                {
                    bit1num += util_bit8_1num(array[i]);
                }
        }
        bit1num += util_bit8_1num(array[i] &( ( (1<<(bitTotal)%8)) - 1));
...
#define  DUMP_PRINTF  xprintf
void  util_hex_dump(p_str tips, p_str data, u_int32 len)
{
	s_int32 i= 0;
    DUMP_PRINTF("0x%X[%d] %s ", data, len, tips);
	if (NULL != data)
	{
		for (i=0;i<len;i++)
		{
		    DUMP_PRINTF(" %02X",data[i]);
