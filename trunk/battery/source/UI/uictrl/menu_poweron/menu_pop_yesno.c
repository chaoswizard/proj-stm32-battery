...
static u_int8 yesno_menu_cell_zone_int(struct OSD_ZONE *zone, T_UICOM_OBJ_COUNT pos);
static u_int8 yesno_menu_cell_data_int(struct OSD_ZONE *zone, PUICOM_DATA item, T_UICOM_OBJ_COUNT pos, T_UICOM_OBJ_COUNT childIdx, enum OSD_OBJ_DRAW_TYPE type);
...
static void yesno_menu_paint(u_int8 isClear)
{
    if (isClear)
    {
       gmenu_content_list_clear(&gPopMenuYesNoMenu, 1, INPUTBOX_LIST_NUM-1, 0);
    }
