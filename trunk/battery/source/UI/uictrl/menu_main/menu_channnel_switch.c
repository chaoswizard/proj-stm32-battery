    ...
    if (pos < CHANNELSW_NUM_PER_PAGE)//通道
    {
        switch (childIdx)
        {
            case 0:
                ...
                break;
            case 1:
                sprintf(UICOM_DATA_BUF(item), "%03d", gMenuChSwitchDataCtrl.curBaseCh + pos);
                break;
            case 2:
...
static u_int8 menu_pub_handle(SM_NODE_HANDLE me, struct EVENT_NODE_ITEM *e)
{
    ...
    switch (e->sig)
    {
        ...
        if (curfocus == IDX_CHANNELSW_SEL_ALL)
        {
            ...
            {
                CHSWITCH_ITEM_SET_STATUS_RANGE(0, CHANNELSW_NUM_PER_PAGE-1);
            }
            else
            {
                CHSWITCH_ITEM_CLR_STATUS_RANGE(0, CHANNELSW_NUM_PER_PAGE-1);
            }
        ...
        {
            channelsw_update_curfocus();
            curfocus = CHSWITCH_ITEM_TST_STATUS_RANGE(0, CHANNELSW_NUM_PER_PAGE-1);
            //MY_DEBUG("Focus NUM:%d\n", curfocus);
            if (0 == curfocus)
            {
                CHSWITCH_ITEM_CLR_STATUS(IDX_CHANNELSW_SEL_ALL);
            }
            else if(CHANNELSW_NUM_PER_PAGE == curfocus)
            {
